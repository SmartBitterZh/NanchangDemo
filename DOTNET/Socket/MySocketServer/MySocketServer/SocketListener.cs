...
namespace MySocketServer
{
  ...
  public class SocketListener : IDisposable
  {
    ...
    protected void LogOutEvent(Object sender, MessageType type, string message)
    {
      if (MessageOut != null)
    ...
    public void Listen(object data)
    {
      while (true)
      {
        string[] keys = m_readWritePool.OnlineUID;
        foreach (string uid in keys)
        {
          if (uid != null && m_readWritePool.m_busypool[uid].ReceiveSAEA.LastOperation != SocketAsyncOperation.Receive)
          {
            bool willRaiseEvent = (m_readWritePool.m_busypool[uid].ReceiveSAEA.UserToken as AsyncUserToken).Socket.ReceiveAsync(m_readWritePool.m_busypool[uid].ReceiveSAEA);
            if (!willRaiseEvent)
              ProcessReceive(m_readWritePool.m_busypool[uid].ReceiveSAEA);
          }
        }
      }
    }
    ...
    private void ProcessAccept(SocketAsyncEventArgs accepter)
    {
      ...
      try
      {
        ...
        {
          LogOutEvent(null, MessageType.Error, string.Format("The Socket Not Connect {0}", accepter.AcceptSocket.RemoteEndPoint));
    ...
    private void ProcessSend(SocketAsyncEventArgs sender)
    {
      ...
      if (sender.BytesTransferred > 0)
      {
        if (sender.SocketError == SocketError.Success)
          OnSended(_uid, MessageType.Success.ToString());
        else
          OnSended(_uid, MessageType.Failed.ToString());
      }
      else
        this.CloseClientSocket(_uid, false);
    }
  ...
  public class LogOutEventArgs : EventArgs
  {
    ...
    /// 消息类型
    /// </summary>     
    private MessageType messClass;
    /// <summary>
    /// 消息类型
    /// </summary>  
    public MessageType MessClass
    {
    ...
    public LogOutEventArgs(MessageType messclass, string str)
    {
      messClass = messclass;
      mess = str;
