...
namespace YYT.Server
{
  ...
  public class DataPortal : IDataPortalServer
  {
    ...
    /// </summary>
    public DataPortal()
      : this("YYTAuthorizationProvider")
    {
      
    }

    /// <summary>
    /// This construcor accepts the App Setting name for the YYT Authorization Provider,
    /// therefore getting the provider type from configuration file
    /// </summary>
    /// <param name="YYTAuthorizationProviderAppSettingName"></param>
    protected DataPortal(string YYTAuthorizationProviderAppSettingName)
      : this(GetAuthProviderType(YYTAuthorizationProviderAppSettingName))
    {
    }
    ...
    protected DataPortal(Type authProviderType)
    {
      if(null==authProviderType)
        throw new ArgumentNullException("authProviderType", Resources.YYTAuthenticationProviderNotSet);
    ...
    private static Type GetAuthProviderType(string YYTAuthorizationProviderAppSettingName)
    {
      if(YYTAuthorizationProviderAppSettingName==null)
        throw new ArgumentNullException("YYTAuthorizationProviderAppSettingName", Resources.AuthorizationProviderNameNotSpecified);


      if (null == _authorizer)//not yet instantiated
      {
        var authProvider = ConfigurationManager.AppSettings[YYTAuthorizationProviderAppSettingName];
