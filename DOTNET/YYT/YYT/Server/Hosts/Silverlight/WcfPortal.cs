   ...
   public class WcfPortal : IWcfPortal
  {
    ...
    public static IMobileFactoryLoader FactoryLoader 
    {
      get
      {
        ...
        {
          string setting = ConfigurationManager.AppSettings["YYTMobileFactoryLoader"];
    ...
    public WcfResponse Fetch(CriteriaRequest request)
    {
      ...
      try
      {
        ...
        {
          if (criteria == null)
            o = YYT.DataPortal.Fetch(t);
          else
            o = YYT.DataPortal.Fetch(t, criteria);
        }
        else
        {
          ...
          object f = FactoryLoader.GetFactory(factoryInfo.FactoryTypeName);
          if (criteria != null)
            o = YYT.Reflection.MethodCaller.CallMethod(f, factoryInfo.FetchMethodName, criteria);
          else
            o = YYT.Reflection.MethodCaller.CallMethod(f, factoryInfo.FetchMethodName);
        }
        result.ObjectData = MobileFormatter.Serialize(o);
        result.GlobalContext = MobileFormatter.Serialize(ApplicationContext.GlobalContext);
      }
    ...
    public WcfResponse Update(UpdateRequest request)
    {
      ...
      try
      {
        ...
        else
        {
          ...
          SetContext(request);
          object f = FactoryLoader.GetFactory(factoryInfo.FactoryTypeName);
          o = YYT.Reflection.MethodCaller.CallMethod(f, factoryInfo.UpdateMethodName, obj);
        }
        result.ObjectData = MobileFormatter.Serialize(o);
        result.GlobalContext = MobileFormatter.Serialize(ApplicationContext.GlobalContext);
      }
    ...
    private static YYT.Silverlight.MobileFactoryAttribute GetMobileFactoryAttribute(Type objectType)
    {
      var result = objectType.GetCustomAttributes(typeof(YYT.Silverlight.MobileFactoryAttribute), true);
      if (result != null && result.Length > 0)
        return result[0] as YYT.Silverlight.MobileFactoryAttribute;
      else
        return null;
    }
    #endregion
