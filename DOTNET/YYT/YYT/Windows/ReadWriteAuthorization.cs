...
namespace YYT.Windows
{
  ...
  /// on the authorization settings from a YYT .NET 
  /// business object.
  /// </summary>
  [DesignerCategory("")]
  [ProvideProperty("ApplyAuthorization", typeof(Control))]
  [ToolboxItem(true), ToolboxBitmap(typeof(ReadWriteAuthorization), "YYT.Windows.ReadWriteAuthorization")]
    ...
    private void ApplyAuthorizationRules(Control control)
    {
      foreach (Binding binding in control.DataBindings)
      {
        ...
        if (binding.DataSource is BindingSource)
        {
          ...
          YYT.Security.IAuthorizeReadWrite ds = 
            bs.Current as YYT.Security.IAuthorizeReadWrite;
          if (ds != null)
          {
